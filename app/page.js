...
export default function Dashboard() {
  ...
  const filtered = useMemo(() => {
    return rows.filter((r) => {
      ...
      if (search) {
        const hay = `${r["STYLE NAME"] || ""} ${r["PO NO"] || ""} ${r["ORDER ID"] || ""}`.toLowerCase();
  ...
  const totals = useMemo(() => {
    const planQty = filtered.reduce((sum, r) => sum + toNumber(r["PLAN QTY"]), 0);
    const cuttingQty = filtered.reduce((sum, r) => sum + toNumber(r["CUTTING QTY"]), 0);
    const pendingQty = planQty - cuttingQty;
    const pct = filtered.map((r) => toNumber(r["QTY CUT IN %"])).filter((n) => n > 0);
    const avgPct = pct.length ? (pct.reduce((a, b) => a + b, 0) / pct.length).toFixed(1) : "0";
  ...
  const orderTypePieData = useMemo(() => {
    const map = {};
    filtered.forEach((r) => { const t = r["ORDER TYPE"] || "—"; map[t] = (map[t] || 0) + 1; });
  ...
  function withPercentShare(summary) {
    ...
    return summary.map((g) => ({ ...g, pctShare: totalPlan > 0 ? (g.planQty / totalPlan) * 100 : 0 }));
  }

  const operationSummaryPct = useMemo(() => withPercentShare(operationSummary), [operationSummary]);
  const styleSummaryPct = useMemo(() => withPercentShare(styleSummary), [styleSummary]);
  ...
  const styleSearchResult = useMemo(() => {
    ...
    const matches = rows.filter((r) => (r["STYLE NAME"] || "") === styleQuery);
    ...
    const pctVals = matches.map((r) => toNumber(r["QTY CUT IN %"])).filter((n) => n > 0);
  ...
  const slicerTotals = useMemo(() => {
    return monthSummaryFiltered.reduce(
      (acc, g) => ({ planQty: acc.planQty + g.planQty, cuttingQty: acc.cuttingQty + g.cuttingQty, pendingQty: acc.pendingQty + g.pendingQty }),
    ...
    <div className="wrap">
      ...
      {status === "error" && (
        <p className="state error">
          ...
          variables and the sheet is shared as "Anyone with the link — Viewer".
        </p>
      )}
      ...
      {status === "ready" && (
        <>
            ...
            {styleSearchResult && (
              <div className="cards" style={{ marginTop: 12, marginBottom: 4 }}>
                ...
                <div className="card">
                  ...
                  <p className="value">{styleSearchResult.planQty.toLocaleString("en-IN")}</p>
                </div>
                <div className="card accent">
                  <p className="label">Cutting qty</p>
                  <p className="value">{styleSearchResult.cuttingQty.toLocaleString("en-IN")}</p>
                </div>
                <div className="card accent">
                  <p className="label">Avg cutting qty / order</p>
                  <p className="value">{styleSearchResult.avgCutting.toLocaleString("en-IN", { maximumFractionDigits: 0 })}</p>
                </div>
                <div className="card accent">
                  <p className="label">Avg qty cut %</p>
                  <p className="value">{styleSearchResult.avgPct.toFixed(1)}%</p>
                </div>
              </div>
            )}
            ...
            <div className="quick-stack">
              <div>
                ...
                <div className="table-wrap" style={{ maxHeight: 260, overflowY: "auto" }}>
                  <table>
                    <thead><tr><th>Cutting date</th><th>Total qty</th></tr></thead>
                    <tbody>
                      {dailyCuttingReport.rows.map((r) => (
                        <tr key={r.date}><td>{r.date}</td><td>{r.cuttingQty.toLocaleString("en-IN")}</td></tr>
                      ))}
                    </tbody>
                  </table>
                </div>
                <p className="row-count">
                  ...
                  {" · "}Avg/day: {dailyCuttingReport.avgPerDay.toLocaleString("en-IN", { maximumFractionDigits: 0 })}
                </p>
              </div>
              <div style={{ marginTop: 22 }}>
                ...
                <div className="table-wrap">
                  <table>
                    <thead><tr><th>Collection</th><th>Plan qty</th><th>Cutting qty</th><th>% share</th></tr></thead>
                    <tbody>
          ...
          <div className="cards">
            <div className="card"><p className="label">Rows in view</p><p className="value">{totals.poCount}</p></div>
            <div className="card"><p className="label">Total plan qty</p><p className="value">{totals.planQty.toLocaleString("en-IN")}</p></div>
            <div className="card accent"><p className="label">Total cutting qty</p><p className="value">{totals.cuttingQty.toLocaleString("en-IN")}</p></div>
            <div className="card pending"><p className="label">Pending qty</p><p className="value">{totals.pendingQty.toLocaleString("en-IN")}</p></div>
            <div className="card accent"><p className="label">Avg qty cut %</p><p className="value">{totals.avgPct}%</p></div>
          </div>
          ...
          <div className="panel">
            <div className="view-tabs">
              <button className={viewMode === "log" ? "active" : ""} onClick={() => setViewMode("log")}>Order log</button>
              <button className={viewMode === "month" ? "active" : ""} onClick={() => setViewMode("month")}>Month-wise pending</button>
              <button className={viewMode === "date" ? "active" : ""} onClick={() => setViewMode("date")}>Date-wise pending</button>
              <button className={viewMode === "operation" ? "active" : ""} onClick={() => setViewMode("operation")}>Operation overview</button>
              <button className={viewMode === "style" ? "active" : ""} onClick={() => setViewMode("style")}>Style-wise</button>
            </div>

            {viewMode === "month" && (
              <div className="slicer">
                {monthSummary.map((g) => {
                  const active = selectedMonths.length === 0 || selectedMonths.includes(g.key);
                  return (
                    <button key={g.key} className={`slicer-pill ${active ? "on" : ""}`} onClick={() => toggleMonthSlicer(g.key)}>
                      {g.key}
                    </button>
                  );
                })}
                {selectedMonths.length > 0 && (
                  <button className="slicer-clear" onClick={() => setSelectedMonths([])}>Clear</button>
                )}
              </div>
            )}
            ...
            <div className="filters">
              ...
              {viewMode === "log" && (
                <input placeholder="Search style, PO no, order id" value={search} onChange={(e) => setSearch(e.target.value)} />
              )}
            </div>
            ...
            {viewMode === "log" && (
              <>
                  ...
                  <table>
                    ...
                    <tbody>
                      {filtered.slice(0, 200).map((row, i) => (
                        <tr key={i}>
                          {columns.map((c) => (
                            <td key={c}>{c === "ORDER TYPE" && row[c] ? <span className="badge">{row[c]}</span> : row[c]}</td>
                          ))}
                        </tr>
                      ))}
                    </tbody>
                ...
                <p className="row-count">Showing {Math.min(filtered.length, 200)} of {filtered.length} rows</p>
              </>
            )}
              ...
              <>
                {selectedMonths.length > 0 && (
                  <div className="slicer-summary">
                    ...
                    {" · "}<strong>Pending {slicerTotals.pendingQty.toLocaleString("en-IN")}</strong>
                  </div>
                )}
                <div className="table-wrap">
                  <table>
                    <thead><tr><th>Month</th><th>Orders</th><th>Plan qty</th><th>Cutting qty</th><th>Pending qty</th></tr></thead>
                    <tbody>
                      {monthSummaryFiltered.map((g) => (
                        <tr key={g.key}>
                          <td>{g.key}</td><td>{g.rows}</td>
                          <td>{g.planQty.toLocaleString("en-IN")}</td>
                          <td>{g.cuttingQty.toLocaleString("en-IN")}</td>
                          <td style={{ color: g.pendingQty > 0 ? "#a32d2d" : "inherit", fontWeight: 700 }}>
                ...
                {selectedMonths.length > 0 && (
                  <div style={{ marginTop: 22 }}>
                    ...
                    <div className="cards" style={{ marginBottom: 14 }}>
                      <div className="card"><p className="label">Month total cutting qty</p><p className="value">{dailyBreakdown.monthTotal?.toLocaleString("en-IN")}</p></div>
                      <div className="card accent"><p className="label">Days with cutting</p><p className="value">{dailyBreakdown.dayCount}</p></div>
                      <div className="card accent"><p className="label">Avg cutting qty / day</p><p className="value">{dailyBreakdown.avgPerDay?.toLocaleString("en-IN", { maximumFractionDigits: 0 })}</p></div>
                    </div>
                    <div className="table-wrap">
                      <table>
                        <thead><tr><th>Cutting date</th><th>Cutting qty</th></tr></thead>
                        <tbody>
            ...
            {viewMode === "date" && (
              <div className="table-wrap">
                <table>
                  <thead><tr><th>Cutting date</th><th>Orders</th><th>Plan qty</th><th>Cutting qty</th><th>Pending qty</th></tr></thead>
                  <tbody>
                    {dateSummary.map((g) => (
                      <tr key={g.key}>
                        <td>{g.key}</td><td>{g.rows}</td>
                        <td>{g.planQty.toLocaleString("en-IN")}</td>
                        <td>{g.cuttingQty.toLocaleString("en-IN")}</td>
                        <td style={{ color: g.pendingQty > 0 ? "#a32d2d" : "inherit", fontWeight: 700 }}>{g.pendingQty.toLocaleString("en-IN")}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
            ...
            {viewMode === "operation" && (
              <div className="table-wrap">
                <table>
                  <thead><tr><th>Operation</th><th>Orders</th><th>Plan qty</th><th>Cutting qty</th><th>% share of plan</th></tr></thead>
                  <tbody>
                    {operationSummaryPct.map((g) => (
                      <tr key={g.key}>
                        ...
                        <td>{g.planQty.toLocaleString("en-IN")}</td>
                        <td>{g.cuttingQty.toLocaleString("en-IN")}</td>
                        <td style={{ fontWeight: 700 }}>{g.pctShare.toFixed(1)}%</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
            ...
            {viewMode === "style" && (
              <div className="table-wrap">
                <table>
                  <thead><tr><th>Style name</th><th>Orders</th><th>Plan qty</th><th>Cutting qty</th><th>% share of plan</th></tr></thead>
                  <tbody>
